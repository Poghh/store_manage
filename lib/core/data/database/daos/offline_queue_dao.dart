...
class OfflineQueueDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<int> enqueue(String queueKey, String payload) {
    return into(offlineQueueTable).insert(
  ...
  Future<void> removeFirst(String queueKey) async {
    final first = await (select(offlineQueueTable)
          ..where((t) => t.queueKey.equals(queueKey))
          ..orderBy([(t) => OrderingTerm.asc(t.id)])
          ..limit(1))
        .getSingleOrNull();
    if (first != null) {
      await (delete(offlineQueueTable)..where((t) => t.id.equals(first.id)))
          .go();
    }
  }

  Future<void> clearQueue(String queueKey) async {
    await (delete(offlineQueueTable)..where((t) => t.queueKey.equals(queueKey)))
        .go();
  }
  ...
  Future<void> markSynced(int id) async {
    await (update(offlineQueueTable)..where((t) => t.id.equals(id)))
        .write(OfflineQueueTableCompanion(syncedAt: Value(DateTime.now())));
  }
}
